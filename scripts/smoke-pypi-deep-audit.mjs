#!/usr/bin/env node
/** Live PyPI deep audit with real Semgrep — guard :3001, semgrep on PATH, GATEWAY_API_KEY for triage. */
...
if (!process.env.GATEWAY_API_KEY) {
  console.error('FAIL GATEWAY_API_KEY required for smoke:pypi-deep-audit (triage phase)');
  process.exit(1);
}
...
const createRes = await fetch(`${base}/v1/guard/audits`, {
  ...
  body: JSON.stringify({ ecosystem: 'pypi', name, version, depth: 'deep' }),
});
const created = await createRes.json();

if (createRes.status !== 202 || !created.audit_id) {
  console.log('FAIL create pypi deep audit', createRes.status, JSON.stringify(created));
  process.exit(1);
}
...
while (job.status === 'pending' || job.status === 'running') {
  if (Date.now() > deadline) {
    console.log('FAIL pypi deep audit timeout', JSON.stringify(job));
    process.exit(1);
  }
...
if (job.status !== 'completed') {
  console.log('FAIL pypi deep audit status', job.status, JSON.stringify(job));
  process.exit(1);
}
...
if (!ok) {
  console.log('FAIL pypi deep audit result', JSON.stringify(job));
  process.exit(1);
}
