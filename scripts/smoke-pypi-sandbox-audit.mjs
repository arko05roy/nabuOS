#!/usr/bin/env node
/** Live PyPI sandbox audit job — guard :3001, Docker+runsc, GATEWAY_API_KEY, semgrep on PATH. */
...
if (!ready.ok || !readyBody.ready) {
  console.log('SKIP pypi sandbox audit smoke — runtime not ready:', JSON.stringify(readyBody));
  process.exit(0);
}
...
const name = process.env.SANDBOX_AUDIT_PYPI_PKG ?? 'requests';
const version = process.env.SANDBOX_AUDIT_PYPI_VERSION ?? '2.31.0';

const createRes = await fetch(`${base}/v1/guard/audits`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ ecosystem: 'pypi', name, version, depth: 'sandbox' }),
});
const created = await createRes.json();

if (createRes.status !== 202 || !created.audit_id) {
  console.log('FAIL create pypi sandbox audit', createRes.status, JSON.stringify(created));
  process.exit(1);
}
...
while (job.status === 'pending' || job.status === 'running') {
  if (Date.now() > deadline) {
    console.log('FAIL pypi sandbox audit timeout', JSON.stringify(job));
    process.exit(1);
  }
...
if (job.status !== 'completed') {
  console.log('FAIL pypi sandbox audit status', job.status, JSON.stringify(job));
  process.exit(1);
}
...
const ok =
  ...
  job.deep_verdict &&
  job.sandbox &&
  job.sandbox.phase === 'pypi_install' &&
...
if (!ok) {
  console.log('FAIL pypi sandbox audit result', JSON.stringify(job));
  process.exit(1);
}

console.log(
  'ok pypi sandbox audit',
  created.audit_id,
  `${name}@${version}`,
